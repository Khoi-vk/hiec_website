  ...
  return (
    <div className="space-y-6 text-left animate-fade-up">
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={4} className="text-center py-20">
                    <Loader2 className="animate-spin mx-auto text-cyan-600 size-10" />
                  </TableCell>
                </TableRow>
              ) : (
                members.map((m) => (
                  <TableRow
                    key={m.id}
                    className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800/50 last:border-0 transition-colors"
                  >
                    <TableCell className="pl-8 py-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-bold text-[#0f3d3e] dark:text-slate-100 transition-colors">
                            {m.fullName}
                          </p>
                          <p className="text-[10px] text-cyan-600 dark:text-cyan-400 font-bold uppercase tracking-widest transition-colors">
                            {m.position}
                          </p>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell className="text-xs font-medium text-slate-500 dark:text-slate-400 transition-colors">
                      {m.department}
                    </TableCell>
                    <TableCell className="text-center font-mono font-bold text-slate-400 dark:text-slate-500 transition-colors">
                      {m.displayOrder}
                    </TableCell>
      ...
      <Modal
        ...
        title={editingId ? "Sửa thông tin" : "Thêm thành viên"}
        description="Thông tin này sẽ hiển thị công khai trên trang Thành viên."
      >
        <div className="space-y-4 py-4 max-h-[75vh] overflow-y-auto pr-2 custom-scrollbar text-left">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              <Input
                ...
                className="rounded-xl bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300 cursor-pointer transition-colors"
              />
            </div>
          </div>
          {formData.avatarUrl && (
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-500 transition-colors">
              Giới thiệu ngắn / Châm ngôn
            </label>
            <Textarea
              value={formData.bio}
              onChange={(e) => setFormData({ ...formData, bio: e.target.value })}
              className="rounded-xl bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-900 dark:text-slate-100 min-h-[80px] transition-colors"
            />
          </div>
          <Button
