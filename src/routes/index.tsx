  ...
  return (
    <PublicLayout>
      <main>
        ...
        <section className="px-5 py-20 lg:px-8 lg:py-28"><div className="mx-auto grid max-w-7xl gap-10 rounded-[2rem] border border-border bg-primary/20 p-8 sm:p-12 lg:grid-cols-[1fr_auto] lg:items-end"><div><p className="text-sm font-black uppercase tracking-[0.22em] text-primary-deep dark:text-primary">Make your move</p><h2 className="mt-4 max-w-2xl text-4xl font-black leading-tight sm:text-6xl"><span className="sm:whitespace-nowrap">Ý tưởng thành tiền thật</span><br />Gia nhập HIEC ngay!</h2><p className="mt-5 max-w-xl text-muted-foreground">Nếu bạn muốn thử sức trong một môi trường nhiều năng lượng, HIEC đang chờ bạn.</p></div><div className="flex flex-wrap gap-3"><Button asChild size="lg" className="rounded-full"><Link to="/signup">Đăng ký tham gia <ArrowUpRight className="ml-2 size-4" /></Link></Button><Button asChild size="lg" variant="outline" className="rounded-full"><a href={`mailto:${contactInfo.email}`}><Mail className="mr-2 size-4" /> Liên hệ</a></Button></div></div></section>
      </main>
    </PublicLayout>
  );
}
